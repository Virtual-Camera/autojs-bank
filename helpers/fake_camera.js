...
let fake_camera_log = globalThis.__LogAxiomSingleton__;
let sendLog = fake_camera_log?.sendLog || null;
...
const select_random_video = function (includeText = [], folder_video = "/sdcard/Movies/FaceData/") {
    try {
        ...
        sendLog(name + "select_random_video from folder: " + folder_video + ", list: " + JSON.stringify(listVideo) + ", includeText: " + JSON.stringify(includeText))
        let filtedVideo = []
        ...
        sendLog(name + "select_random_video: " + e)
        return false
    }
...
const change_ramdom_video = function (includeName = [], folder_video = "/sdcard/Movies/FaceData/") {
    ...
    if (pathVideo) {
        if (pathVideo.includes("undefined")) {
            sendLog(name + "pathVideo is undefined", "warn")
            showText("Can not select random video", 150, 1180, 50, "#0000FF", 1000)
            return false
        }
    } else {
        sendLog(name + "Can not select random video", "warn")
        return false
    }
    sendLog(name + "pathVideo: " + pathVideo)
    sendLog(name + "change_ramdom_video: " + pathVideo)
