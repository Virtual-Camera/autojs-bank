...
function connect(config, deviceSN) {
    ...
    logAxiom("Connecting WS to channel: " + config.channel);
    ...
    try {
        ...
        logAxiom("Error creating WebSocket: " + e);
        isConnecting = false;
        scheduleReconnect();
    }
}
    ...
    socket.on("text", function (text, ws) {
        ...
        try {
            ...
            if (json.data && json.data.customData && json.data.customData.sn) {
                ...
                logAxiom("Received SN: " + receivedSN); // Debug

                if (receivedSN === currentDeviceSN) {
                    logAxiom("--> MATCH SN (" + currentDeviceSN + "). Processing...");
                    // Gọi sang module Processor
                    Processor.handle(json);
                } else {
                    logAxiom("--> MISMATCH SN. Device SN: " + currentDeviceSN + ", Message SN: " + receivedSN);
                }
            } else {
                // Các tin hệ thống hoặc không đúng format
                if (json.event) logAxiom("Event: " + json.event);
            }

        } catch (e) {
            logAxiom("Error parsing JSON: " + e);
        }
    });

    socket.on("closing", function (code, reason, ws) {
        logAxiom("WS Closing: " + reason);
        stopHeartbeat();
    });

    socket.on("closed", function (code, reason, ws) {
        logAxiom("WS Closed. Reconnecting...");
        isConnecting = false;
        stopHeartbeat();
        scheduleReconnect();
    });
